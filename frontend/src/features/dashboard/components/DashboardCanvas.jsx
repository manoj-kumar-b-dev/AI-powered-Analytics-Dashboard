...
export function DashboardCanvas({
  ...
  if (!widgets || widgets.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-12 rounded-2xl border border-dashed border-slate-300 dark:border-[#1F2937] bg-white/60 dark:bg-[#111827]/10 min-h-[400px] backdrop-blur-sm shadow-sm">
        ...
        <h3 className="text-base font-bold text-slate-900 dark:text-white font-display">No widgets added yet</h3>
        <p className="text-slate-500 dark:text-gray-400 text-xs mt-1 max-w-sm">
          ...
          {widgets.map((widget) => {
            ...
            return (
              <div key={widget.id} className="relative">
                ...
                >
                  {widget.error ? (
                    <div className="flex flex-col items-center justify-center h-full text-rose-500 dark:text-rose-400 gap-2 p-4 text-center">
...
function AnimatedNumber({ value }) {
  ...
  useEffect(() => {
    ...
    if (stringVal.endsWith("Cr")) {
      setDisplayVal(stringVal);
      return;
    }
    ...
    const animate = (currentTime) => {
      ...
      if (Math.abs(numericVal) >= 10000000) {
        const crVal = current / 10000000;
        const formatted = crVal.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
        setDisplayVal(`${formatted} Cr`);
      } else {
      ...
      if (progress < 1) {
        ...
        if (Math.abs(numericVal) >= 10000000) {
          const crVal = numericVal / 10000000;
          setDisplayVal(`${crVal.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} Cr`);
        } else {
          setDisplayVal(stringVal.replace(/[₹\$]/g, '').trim());
        }
      }
    };
    ...
    <div className="h-full flex flex-col justify-between py-1 select-text">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col text-left flex-1 min-w-0">
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold tracking-wider uppercase font-display truncate">
            {title || kpiType || "Value"}
          </span>
          <h2 className="text-3xl font-extrabold font-display text-slate-900 dark:text-white tracking-tight mt-1">
      ...
      {hasTrend && (
        <div className="flex items-center justify-between mt-3 gap-2">
          <div className="flex items-center gap-1.5">
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-0.5 ${isPositive ? "bg-[#22C55E]/10 text-emerald-600 dark:text-[#22C55E]" : "bg-[#EF4444]/10 text-rose-600 dark:text-[#EF4444]"
              }`}>
              {isPositive ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              <span>{trendLabel}</span>
            </span>
            <span className="text-[9px] text-slate-400 dark:text-slate-400 font-medium">{timeframeLabel}</span>
          </div>
      ...
      {/* Bottom: progress bar */}
      {progressPct !== null && (
        <div className="mt-4 border-t border-slate-200 dark:border-[#1F2937]/30 pt-3 flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-[9px] font-bold text-slate-500 dark:text-gray-500 uppercase tracking-wider">
            <span>Change vs Prior</span>
            <span className={isPositive ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>{trendLabel}</span>
          </div>
          <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden border border-slate-200 dark:border-white/[0.02]">
            <div
              className={`h-full rounded-full bg-gradient-to-r ${isPositive ? "from-[#8B5CF6] to-[#6366F1]" : "from-[#EF4444] to-[#F59E0B]"
                }`}
      ...
      {/* No-data fallback */}
      {!hasTrend && (
        <p className="text-[9px] text-slate-400 dark:text-slate-400 font-medium mt-2">
...
function WidgetTable({ data }) {
  if (!data || data.length === 0) {
    return <div className="text-slate-400 dark:text-gray-500 text-xs text-center py-8 font-medium">No data sets available</div>;
  }

  const maxVal = Math.max(...data.map(r => typeof r.y === "number" ? r.y : 0), 1);

  return (
    <div className="overflow-x-auto overflow-y-auto max-w-full w-full max-h-[160px] rounded-xl border border-slate-200 dark:border-[#1F2937]/50 bg-slate-50/50 dark:bg-slate-950/20 select-text custom-scrollbar">
      <table className="w-full text-[11px] text-left border-collapse font-sans">
        <thead>
          <tr className="border-b border-slate-200 dark:border-[#1F2937]/50 bg-slate-100/70 dark:bg-slate-900/40 text-slate-600 dark:text-gray-400 font-bold uppercase tracking-wider select-none">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-[#1F2937]/20">
          {data.slice(0, 10).map((row, idx) => {
            const val = row.y;
            const barWidth = Math.min(100, Math.round((val / maxVal) * 100));

            return (
              <tr key={idx} className="hover:bg-slate-100/40 dark:hover:bg-white/[0.01] transition-colors">
                <td className="px-3.5 py-2 text-slate-800 dark:text-gray-300 font-medium truncate max-w-[120px]">{row.x}</td>
                <td className="px-3.5 py-2">
                  <div className="flex items-center gap-2.5 min-w-[80px]">
                    <div className="h-1.5 w-16 bg-slate-200 dark:bg-slate-950 rounded-full overflow-hidden shrink-0">
                      <div className="h-full bg-[#8B5CF6] rounded-full" style={{ width: `${barWidth}%` }} />
                    </div>
                    <span className="text-[10px] text-slate-500 dark:text-gray-400 font-bold font-mono">{barWidth}%</span>
                  </div>
                </td>
                <td className="px-3.5 py-2 text-right text-indigo-600 dark:text-indigo-400 font-bold font-mono">
